...
import { inviteNewUserToCompany, deleteCompanyUser, appointAsAdmin, getAllUsersOfCompany, editAuthorizations } from '../../../../../../actions/company';
const { Paragraph } = Typography;
const { SubMenu } = Menu;

const ManagerUsers = ({ selectedCompany }) => {
  ...
  const handleAdminModalOk = e => {
    ...
    dispatch(appointAsAdmin(selectedUserId, obj)).then(() => {
      dispatch(getAllUsersOfCompany(selectedCompany._id));
    });
    setAdminModal(false);
  };
  ...
  const handleAuthorizationModalOk = e => {
    if (userAuthRoles.length === 0) {
      ...
      dispatch(editAuthorizations(body, selectedUserId)).then(() => {
        dispatch(getAllUsersOfCompany(selectedCompany._id));
      });
              ...
              <Card style={{ border: '2px solid black' }}>
                <Row className="my-2">
                  <Col xs={4} sm={4} md={2} lg={2}>
                    <Avatar size={64} className="user_svg" src={user?.profileImage && `/${user?.profileImage}`} icon={<UserOutlined />} />
                  </Col>
                  ...
                  <Col xs={1} sm={1} md={1} lg={1} >
                    <Menu mode='vertical' className="anchorStyle">
                      <SubMenu title={
                        <span >
