...
const CompanyBanner = ({ companyDetails }) => {
  ...
  return (
    <Fragment>
      <Row style={{ marginBottom: '10px', height: '330px', width: '100%' }}>
        <Col xs={24} sm={24} md={24} lg={24} style={{ textAlign: 'center' }}>
          {companyDetails && companyDetails.logo && companyDetails.logo !== undefined && <img className='logo-image' height='100px' width='100px' src={`/${companyDetails && companyDetails.logo}`} alt='company logo' />}
          <br />
          {companyDetails && companyDetails.bannerImage && companyDetails.bannerImage !== undefined  && <img className='banner-image' height='300px' width='100%' src={`/${companyDetails && companyDetails.bannerImage}`} alt='company banner' />}
        </Col>
      </Row>
