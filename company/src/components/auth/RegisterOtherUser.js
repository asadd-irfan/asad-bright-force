...
import { Input, Checkbox, Button, Form, Select, Row, Col, Typography, notification } from 'antd';
import { useSelector, useDispatch } from 'react-redux'
import { Redirect } from 'react-router-dom';
import PhoneInput from "react-phone-input-2";
import "react-phone-input-2/lib/style.css";
import styles from './Auth.module.scss'
import { registerOtherUser } from '../../actions/auth';
...
const RegisterOtherUser = () => {
	...
	useEffect(() => {
		if (decodedToken && decodedToken.user) {
			...
			setCompanyId(decodedToken.user.companyId);
			setAuthRoles(decodedToken.user.authorizations);
		}
	}, [decodedToken])
	...
	const handleSubmit = values => {
		if (userPassword.score < 2) {
			...
			let data = { ...obj, phone: isPhone, password: userPassword.password }
			// console.log('data', data)

			dispatch(registerOtherUser(data));
		}
	};
						...
						<div className={styles.userFormWrapper}>
							<Fragment>
								<Form
									...
									className='mb-5'
								>
									<Title level={2} className='text-center'>Hi, {userEmail}</Title>
									<Title level={2} className='text-center'>Welcome to BrightForce</Title>
									<Title level={2} className='text-center'>Where recruitment is reinvented </Title>
									...
									<div className={styles.p10}>
										...
										<Form.Item label='Title' name='title' rules={
											[
												{
													required: true,
													message: 'Please input your Company Title!'
												}
											]
										}>
											{/* <Select
