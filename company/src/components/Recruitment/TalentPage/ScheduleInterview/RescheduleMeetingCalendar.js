...
import { rescheduleInterviewWithTalent } from "../../../../actions/talent";
...
const MeetingCalendar = ({ timezoneName }) => {
  ...
  const rescheduleInterviewFlag = useSelector(
    (state) => state.positions.rescheduleInterviewFlag
  );
  ...
  const handleDateClick = (dateArg) => {
    calendarEvents.map((event) => {
      ...
      // let selectedDate = moment(dateArg.date).format('MM-DD-YYYY HH:mm');
  ...
  const handleSubmit = (values) => {
    ...
    dispatch(
      rescheduleInterviewWithTalent(obj, selectedPositionRecruitment?._id)
    ).then(() => {
      setShowScheduleInterviewModal(false);
    });
  };
  ...
  return (
    <div>
      <Row style={{ border: "1px solid" }}>
        ...
        >
          <div>
            <h5>Reschedule an Interview</h5>
            <p>
              choose when would you like to reschedule an interview with the
              talent
            </p>
          </div>
        </Col>
      ...
      >
        ...
        >
          <div style={{ textAlign: "center" }}>
            <h5>Reschedule Interview with {selectedTalent?.name}</h5>
          </div>
          <hr />
          <div>
            <Form.Item
