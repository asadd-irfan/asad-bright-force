...
import { sendOfferToTalent, getRecruitmentCandidatesList } from '../../../../actions/positions'

const { TextArea } = Input;
const { Option } = Select;

function NewRequestState({ talentName }) {
  ...
  const handleSubmit = (values) => {
    if (values.performanceBonus > 100 || values.performanceBonus < 0) {
      ...
      dispatch(sendOfferToTalent(values, posRecId)).then(() => {
...
export default NewRequestState;
