    ...
    return (
        <div>
            <Card style={{ margin: "12px 0px", minHeight: '300px' }}>
                ...
                <Row  className='mt-3'>
                <Paragraph>{user?.name} is  ready to work a {
                            appConfigsEmploymentType && employmentType &&
                            convertConfigsIdToName(appConfigsEmploymentType, employmentType).map(item => {
                                return <>  {item}</>

                            })
                        } position between:</Paragraph>

                    <Col xs={24} sm={24} md={24} lg={24}>
                    <Paragraph > <span style={{ color: 'red' }}>* </span>   Note the following hours are presented according to your timezone is UTC {timezoneUTC} {timezoneName}</Paragraph>
                    </Col>
                    <Col xs={24} sm={24} md={24} lg={24} className="my-3 py-2">

                    {user && user.workingHours && <TimeRangeSlider
                      disabled={false}
                      minValue={"00:00"}
                      maxValue={"23:59"}
                      format={24}
                    ...
                    //   value={{ start: startHour ? startHour : "09:00", end: endHour  ? endHour : "18:00" }}
                      ...
                      step={60}
                    />}
                    <Paragraph>Start Time: {startHour} , End Time: {endHour}</Paragraph>
