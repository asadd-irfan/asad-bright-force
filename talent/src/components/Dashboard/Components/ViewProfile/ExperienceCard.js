...
function ExperienceCard() {
  const user = useSelector(state => state.auth.user);
    ...
    const preferredRoles = useSelector(state => state.auth.user && state.auth.user.preferredRoles);
    ...
    useEffect(() => {
        ...
        if (preferredRoles && preferredRoles.mainRole && preferredRoles.mainRole.skills) {
        ...
        if (preferredRoles && preferredRoles.secondaryRole && preferredRoles.secondaryRole.skills) {
            appConfigTechExp && appConfigTechExp.map(techExp => {
    ...
    return (
        <div>
            ...
            <Row >
                ...
                <Title level={2} className='mb-4'>Professional Experience</Title>
                    <Row>
                    {/* <h5>Main Role: </h5> */}
                    <Tag color="blue" className="font-weight-bold mb-4"> {mainRoleName}</Tag>
                    {preferredRoles?.mainRole?.yearsOfExperience !==  null && <Paragraph className="font-weight-bold mb-4">{preferredRoles?.mainRole?.yearsOfExperience} year of Experience</Paragraph > }

                    </Row>
                    <Row>
                    {/* <h5>Secondary Role: </h5> */}
                    <Tag color="blue" className="font-weight-bold mb-4"> {secondaryRoleName}</Tag>
                ...
                <Col xs={24} sm={24} md={12} lg={12}>
                <Title level={2} className='mb-4'>Languages</Title>
                {languages && languages.map(lang => (<>
                     <Tag  className="font-weight-bold mb-4"> {lang.languageName}</Tag> <br /></>
                    ))}
